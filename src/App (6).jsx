...
//    org | title | cause | neighborhood | commitment | days | description | url
//
// 2. Fill in your listings (one per row). For "days", separate with commas:
//    e.g. "Monday, Wednesday, Saturday"
//
// 3. Go to File → Share → Publish to web → select "Comma-separated values (.csv)" → Publish
//
...
function parseCSV(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '"') {
      ...
      if (lines.length) yield lines.splice(0);
      if (ch === "\r" && text[i + 1] === "\n") i++;
    } else {
      current += ch;
    }
...
function* csvGenerator(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (ch === '"') {
      ...
      fields.push(current.trim());
      current = "";
      if (fields.some(f => f)) yield fields;
      fields = [];
      if (ch === "\r" && text[i + 1] === "\n") i++;
    } else {
      current += ch;
...
function sheetToListings(csvText) {
  const rows = [...csvGenerator(csvText)];
  if (rows.length < 2) return [];
  
  const headers = rows[0].map(h => h.toLowerCase().trim());
  const iOrg = headers.indexOf("org");
  const iTitle = headers.indexOf("title");
  const iCause = headers.indexOf("cause");
  const iHood = headers.indexOf("neighborhood");
  const iCommit = headers.indexOf("commitment");
  const iDays = headers.indexOf("days");
  ...
  return rows.slice(1).map((row, idx) => ({
    ...
    title: row[iTitle] || "",
    cause: row[iCause] || "",
    neighborhood: row[iHood] || "",
    commitment: row[iCommit] || "",
    days: (row[iDays] || "").split(",").map(d => d.trim()).filter(Boolean),
    description: row[iDesc] || "",
    url: row[iUrl] || "#",
  })).filter(l => l.org && l.title);
}

// ── FALLBACK DATA (used if Google Sheet isn't connected yet) ─
...
// ── HOOK: Fetch listings from Google Sheet (falls back to hardcoded data) ──
function useListings() {
  const [listings, setListings] = useState(FALLBACK_LISTINGS);
  const [source, setSource] = useState("local");

  useEffect(() => {
    if (!GOOGLE_SHEET_CSV_URL || GOOGLE_SHEET_CSV_URL === "YOUR_GOOGLE_SHEET_CSV_URL_HERE") return;

    fetch(GOOGLE_SHEET_CSV_URL)
      .then(r => r.text())
      .then(csv => {
        const parsed = sheetToListings(csv);
        if (parsed.length > 0) {
          setListings(parsed);
          setSource("sheet");
        }
...
const NEIGHBORHOODS = [
  "Anacostia", "Brookland", "Buzzard Point", "Capitol Hill", "Citywide",
  "Columbia Heights", "Congress Heights", "Downtown", "Foggy Bottom",
  "Petworth", "Rock Creek Park", "Shaw", "Shepherd Park", "Tenleytown", "Virtual",
];
...
function Reveal({ children, className = "", variant = "reveal", delay = 0 }) {
  ...
  return (
    <div ref={ref} className={`${variant} ${className}`} style={delay ? { transitionDelay: `${delay}s` } : undefined}>
      {children}
    </div>
  );
}
...
// Stagger children cards as they scroll into view
function useStaggerCards(containerRef) {
  useEffect(() => {
    ...
    const obs = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          const cards = container.querySelectorAll('.stagger-card:not(.visible)');
...
// Stagger cause chips
function useStaggerChips(containerRef) {
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const obs = new IntersectionObserver(([entry]) => {
      if (entry.isIntersecting) {
        const chips = container.querySelectorAll('.cause-chip');
        chips.forEach((chip, i) => {
          setTimeout(() => chip.classList.add('chip-visible'), i * 60);
        });
...
export default function ShowUpDC() {
  ...
  useStaggerCards(cardsRef);
  useStaggerChips(chipsRef);
        ...
        {page === "home" && (
          <>
            ...
            <section className="hero">
              ...
              <div className="hero-stats hero-float">
                <div className="hero-stat">
                  <div className="hero-stat-num"><AnimatedCount target={LISTINGS.length} suffix="+" /></div>
                  <div className="hero-stat-label">Opportunities</div>
                </div>
                <div className="hero-stat">
                  <div className="hero-stat-num"><AnimatedCount target={new Set(LISTINGS.map(l => l.neighborhood)).size} /></div>
                  <div className="hero-stat-label">Neighborhoods</div>
                </div>
                <div className="hero-stat">
                  <div className="hero-stat-num"><AnimatedCount target={CAUSES.length} /></div>
            ...
            <div className="causes-strip">
              <Reveal><h3>Browse by cause</h3></Reveal>
              <div className="causes-row" ref={chipsRef}>
                {CAUSES.map((c) => (
                  <button
                    key={c}
          ...
          <Reveal>
            <section className="form-section">
              ...
              {formSubmitted ? (
                ...
                <div className="form-grid" onSubmit={(e) => {
                  e.preventDefault();
                  const form = e.target.closest('.form-grid').querySelectorAll('input, select, textarea');
                  const data = {};
                  form.forEach(el => { if (el.name) data[el.name] = el.value; });
                  fetch("https://formspree.io/f/xyknjegr", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(data),
                  }).then(r => {
                    if (r.ok) setFormSubmitted(true);
                  }).catch(() => setFormSubmitted(true));
                }}>
                  <div className="form-row">
                    <div className="form-field">
                      <label>Organization Name</label>
                      <input type="text" name="organization" placeholder="e.g. DC Central Kitchen" required />
                    </div>
                    <div className="form-field">
                      <label>Contact Email</label>
                      <input type="email" name="email" placeholder="you@org.com" required />
                    </div>
                  </div>
                  <div className="form-field">
                    <label>Opportunity Title</label>
                    <input type="text" name="title" placeholder="e.g. Weekend Meal Prep Volunteer" required />
                  </div>
                  <div className="form-row">
                    <div className="form-field">
                  ...
                  <div className="form-field">
                    ...
                    <input type="text" name="days" placeholder="e.g. Monday, Wednesday, Saturday" />
                  </div>
                  <div className="form-field">
                    <label>Description</label>
                    <textarea name="description" placeholder="Tell us about the opportunity — what will volunteers do? What should they expect?" required />
                  </div>
                  <button className="form-submit" onClick={(e) => {
                    const grid = e.target.closest('.form-grid');
                    const inputs = grid.querySelectorAll('input, select, textarea');
                    let valid = true;
                    inputs.forEach(el => { if (el.required && !el.value) { el.focus(); valid = false; } });
                    if (!valid) return;
                    const data = {};
                    ...
                    fetch("https://formspree.io/f/xyknjegr", {
                      ...
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify(data),
                    }).then(r => { if (r.ok) setFormSubmitted(true); })
                      .catch(() => setFormSubmitted(true));
                  }}>
                    Submit for Review
                  </button>
